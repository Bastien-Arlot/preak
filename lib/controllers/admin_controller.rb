    ...
    def adminDashboardControl
        ...
        case params
            when 1
                self.create_item
            when 2
                self.show_items(ItemController.new.allSorted)
            when 3
                self.show_items(@item_controller.sortByAscending(ItemController.new.allSorted, 2))
            when 4
                self.show_items(@item_controller.sortByDescending(ItemController.new.allSorted, 2))
            when 5
                self.update_item
            when 6
                self.delete_item
